...
-- 22
SELECT MIN(imdb_rating) AS min, MAX(imdb_rating) max FROM movies;
-- 23
...
-- 26
SELECT ROUND(SUM(imdb_rating), 2) FROM movies;
-- 27
SELECT * FROM movies WHERE imdb_rating > (SELECT AVG(imdb_rating) FROM movies);
-- 28
SELECT * FROM movies WHERE id NOT BETWEEN 2 AND 4;
-- 29
SELECT m.title, d.name FROM directors d INNER JOIN movies m WHERE m.id = d.id;
-- 30
SELECT m.title, s.name FROM movies_stars JOIN movies m ON m.id = movies_id JOIN stars s WHERE s.id = stars_id;
-- 31
SELECT m.title, g.name FROM movies_genres JOIN movies m ON m.id = movies_id JOIN genres g WHERE g.id = genres_id;
-- 32
SELECT DISTINCT m.title, g.name, d.name FROM movies m 
JOIN movies_genres mg ON mg.movies_id = m.id
JOIN genres g ON g.id = mg.genres_id
JOIN movies_stars ms ON ms.movies_id = m.id
JOIN stars s ON s.id = ms.stars_id
JOIN movies_directors md ON md.movies_id = m.id
JOIN directors d ON d.id = md.directors_id ORDER BY m.title;
-- 33
SELECT m.title, COUNT(g.name) AS genre_count
FROM movies m
JOIN movies_genres mg ON mg.movies_id = m.id
JOIN genres g ON mg.genres_id = g.id
GROUP BY m.title;
-- 34
SELECT d.name, COUNT(m.id) AS TOTAL_PELICULAS FROM movies m
INNER JOIN movies_directors md
ON m.id = md.movies_id
INNER JOIN directors d 
ON d.id = md.directors_id
GROUP BY (d.name);
-- 35
SELECT title,
  CASE
    WHEN AVG(imdb_rating) < 5 THEN 'Not good'
    WHEN AVG(imdb_rating) BETWEEN 5 AND 6 THEN 'Sufficient'
    WHEN AVG(imdb_rating) BETWEEN 7 AND 8 THEN 'Good'
    WHEN AVG(imdb_rating) > 9 THEN 'Excellent'
  END AS rating_description
FROM movies GROUP BY id;